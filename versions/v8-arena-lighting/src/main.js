...
function buildFoeBlob(cfg) {
  // spread foes around the arena, not lined up at z=0
  const z = (cfg.z != null) ? cfg.z : (Math.random() - 0.5) * 12;
  const xJitter = (Math.random() - 0.5) * 2;
  const f = new Blob(cfg.color, new THREE.Vector3(cfg.x + xJitter, 1.8, z));
...
function enterBattle() {
  ...
  // shady but readable night-city arena
  scene.background = new THREE.Color(0x1a1a35);
  scene.fog = new THREE.Fog(0x1a1a35, 18, 55);
  renderer.setClearColor(0x1a1a35);
  groundMaterial.color.set(0x3a3a55);
  // dim daytime lights, add cool moonlight; brighter than v7 so you can see
  ...
  hemi.intensity = 0.55;
  hemi.color.set(0xa0b0ff);
  hemi.groundColor.set(0x2a2855);
  sun.intensity = 0.95;
  sun.color.set(0xc8d4ff);
  ...
  // warm streetlamp point lights scattered around the arena (more + brighter)
  battle.lamps = [];
  const lampSpots = [
    [-6, 5, -4], [-2, 5, -6], [3, 5, -5], [7, 5, -3], [10, 5, 2],
    [8, 5, 5],  [3, 5, 6],   [-2, 5, 5], [-7, 5, 2], [0, 5, 0],
  ];
  for (const [lx, ly, lz] of lampSpots) {
    const lamp = new THREE.PointLight(0xffcc66, 2.6, 18, 1.2);
  ...
  // bright spotlight directly above the player so they can never get lost
  battle.playerSpot = new THREE.PointLight(0xffffff, 2.5, 12, 1.5);
  battle.playerSpot.position.set(blob.physBody.position.x, 6, blob.physBody.position.z);
  scene.add(battle.playerSpot);
...
function exitBattle(won) {
  ...
  if (battle.playerSpot) {
    scene.remove(battle.playerSpot);
    battle.playerSpot = null;
  }
...
// Capture phase so we beat any focused button to the event (otherwise SPACE
// re-triggers whatever button you last clicked, and arrows can fall through to
// browser defaults).
...
function onKeyUp(e) {
  ...
  if (k === ' ' || e.code === 'Space') spaceWasDown = false;
}
window.addEventListener('keydown', onKeyDown, true);
window.addEventListener('keyup', onKeyUp, true);

// release focus from every button after click so arrow keys don't get hijacked
...
function animate() {
  ...
  if (!battle.active) {
    ...
    // follow-light tracks the player so they're always lit
    if (battle.playerSpot) {
      battle.playerSpot.position.set(
        blob.physBody.position.x,
        blob.physBody.position.y + 4,
        blob.physBody.position.z
      );
    }
